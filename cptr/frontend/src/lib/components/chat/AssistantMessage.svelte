				...
				{:else if displayItem.type === 'artifact_item'}
					...
					>
						<div class="flex items-center gap-1.5 px-3 pt-2 pb-0.5">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-3.5 text-gray-400 dark:text-gray-500 shrink-0">
								<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
							</svg>
							<span class="text-sm font-medium text-gray-800 dark:text-gray-100">{artifact.title || 'Artifact'}</span>
						</div>
						{#if preview}
							<p class="px-3 pb-2 pt-0.5 text-xs text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed">{preview}{preview.length >= 200 ? '...' : ''}</p>
						{/if}
					</button>
