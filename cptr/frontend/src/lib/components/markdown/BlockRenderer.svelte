...
{#each tokens as token}
	...
	{:else if token.type === 'table'}
		...
		<div class="not-prose relative w-full group/table">
			<div class="overflow-x-auto max-w-full rounded-lg border border-gray-200/60 dark:border-white/8">
				<table class="w-full text-[13px] text-left text-gray-600 dark:text-gray-300">
					<thead class="text-xs text-gray-500 dark:text-gray-400 uppercase bg-gray-50/80 dark:bg-white/[0.03]">
						<tr>
					...
					<tbody>
						{#each table.rows as row, rowIdx}
							<tr class="bg-white dark:bg-transparent">
								{#each row as cell, i}
									<td
										class="px-3 py-1.5 {rowIdx < table.rows.length - 1 ? 'border-b border-gray-100/60 dark:border-white/5' : ''}"
										style={table.align[i] ? `text-align:${table.align[i]}` : ''}
									>
										<InlineRenderer items={cell.tokens} />
									</td>
